...
function DescriptionSection() {
  ...
  // helper to compute compact initials from title (e.g., "Real-time Urban Insights" -> "RU")
  const initialsFromTitle = (title) => {
    if (!title) return '';
    // split on spaces and hyphens, filter out short words
  ...
  const descriptions = [
    {
      id: 1,
      title: "Real-time Urban Insights",
      text: "EchoNet sensors continuously monitor sound and vibration, providing live data for unparalleled urban planning and environmental awareness.",
      color: "bg-blue-600",
    },
    {
      id: 2,
      title: "Empowering Communities",
      text: "Anyone can deploy a low-cost sensor and earn $ECHO tokens, turning local contributions into a stake in the decentralized data economy.",
      color: "bg-orange-600",
    },
    {
      id: 3,
      title: "AI-Powered Predictions",
      text: "On-device AI classifies sounds and predicts events, from festival disruptions to seismic activity, making cities smarter and safer.",
      color: "bg-purple-600",
    },
  ];
  ...
  useEffect(() => {
    ...
    const stepBack = () => {
      if (!video) return;
      // step back a small amount for a smooth reverse effect
    ...
    const handleEnded = () => {
      // try negative playbackRate first
      try {
        video.pause();
        // some browsers don't allow negative playbackRate; fallback to manual stepping
        video.playbackRate = -1;
        // attempt play; if it throws or doesn't work, fallback below
        const p = video.play();
        if (p && p.catch) {
          p.catch(() => {
            video.playbackRate = 1;
      ...
      <div className="relative z-10 w-full flex flex-col items-center">
        ...
        <div className="text-center mb-16">
          ...
          {descriptions.map((desc, index) => (
            <div 
              key={desc.id} 
              className="group relative rounded-2xl overflow-hidden transform hover:scale-105 transition-all duration-500 hover:z-10"
              style={{
            ...
            >
              ...
              {/* subtle non-animated frame matching navbar glass */}
              ...
              {/* Main card */}
              <div className="relative rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20 shadow-lg overflow-hidden transition-all duration-500">
                ...
                <div className="relative z-10 p-8 text-white">
                  ...
                  {/* Bottom accent line: animate to solid white on hover */}
